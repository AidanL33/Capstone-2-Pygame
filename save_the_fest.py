...
bouncer1 = pygame.image.load("bouncer1.jpeg")
bouncer2 = pygame.image.load("bouncer2.jpeg")
bouncer3 = pygame.image.load("bouncer3.jpeg")
...
bouncer1_height = bouncer1.get_height()
bouncer1_width = bouncer1.get_width()

bouncer2_height = bouncer2.get_height()
bouncer2_width = bouncer2.get_width()

bouncer3_height = bouncer3.get_height()
bouncer3_width = bouncer3.get_width()
...
bouncer1XPosition =  screen_width
bouncer1YPosition =  random.randint(0, screen_height - bouncer1_height)
bouncer2XPosition =  screen_width
bouncer2YPosition =  random.randint(0, screen_height - bouncer2_height)
bouncer3XPosition =  screen_width
bouncer3YPosition =  random.randint(0, screen_height - bouncer3_height)
...
while 1: # This is a looping structure that will loop the indented code until you tell it to stop(in the event where you exit the program by quitting). In Python the int 1 has the boolean value of 'true'. In fact numbers greater than 0 also do. 0 on the other hand has a boolean value of false. You can test this out with the bool(...) function to see what boolean value types have. You will learn more about while loop structers later. 
    ...
    screen.blit(bouncer1, (bouncer1XPosition, bouncer1YPosition))
    screen.blit(bouncer2, (bouncer2XPosition, bouncer2YPosition))
    screen.blit(bouncer3, (bouncer3XPosition, bouncer3YPosition))  
    ...
    enemyBox = pygame.Rect(bouncer1.get_rect())
    enemyBox.top = bouncer1YPosition
    enemyBox.left = bouncer1XPosition

    enemyBox2 = pygame.Rect(bouncer2.get_rect())
    enemyBox2.top = bouncer2YPosition
    enemyBox2.left = bouncer2XPosition

    enemyBox3 = pygame.Rect(bouncer3.get_rect())
    enemyBox3.top = bouncer3YPosition
    enemyBox3.left = bouncer3XPosition
    ...
    #collision with prize box
    if playerBox.colliderect(prizeBox):
        print("The DJ saved the festival!")
        pygame.quit()
        exit(0)
    ...
    if bouncer1XPosition < 0 - bouncer1_width:
        if bouncer2XPosition < 0 - bouncer2_width:
            if bouncer3XPosition < 0 - bouncer3_width:
    ...
    bouncer1XPosition -= 0.15
    bouncer2XPosition -= 0.20
    bouncer3XPosition -= 0.25
